...
func TestConnTrackSystem_DumpFlows(t *testing.T) {
	...
	nonAntreaFlow := connectionstest.NewBuilder().SetDestinationAddress(gwAddr).SetZone(100).Get()
	testVarietyFlows := []*flowexporter.Connection{antreaFlow, antreaServiceFlow, antreaGWFlow, nonAntreaFlow}
	antreaUPDFlow := connectionstest.NewBuilder().SetProtocol(17).Get()
	antreaSCTPFlow := connectionstest.NewBuilder().SetProtocol(132).Get()
...
func TestConnTrackOvsAppCtl_DumpFlows(t *testing.T) {
	...
	expConn := connectionstest.NewBuilder().SetTimeout(86399).
		...
		SetLabels([]byte{1, 0, 0, 0, 2, 0, 0, 0}).Get()
...
func TestNetLinkFlowToAntreaConnection(t *testing.T) {
	...
	builder := connectionstest.NewBuilder().
		...
		SetReverseBytes(netlinkFlow.CountersReply.Bytes)
	expectedAntreaFlow := builder.Get()
