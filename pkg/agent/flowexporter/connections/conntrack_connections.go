...
func (cs *ConntrackConnectionStore) AddOrUpdateConn(conn *connection.Connection) {
	...
	if exists {
		klog.InfoS("addorupdateconn received - conn exists", "conn", conn)
		existingConn.IsPresent = conn.IsPresent
		if utils.IsConnectionDying(existingConn) {
			klog.InfoS("addorupdateconn received - conn dying", "conn", conn)
			return
		}
		...
		if conn.SourcePodName == "" && conn.DestinationPodName == "" {
			...
			klog.InfoS("Skip this connection as we cannot map any of the connection IPs to a local Pod", "srcIP", conn.FlowKey.SourceAddress.String(), "dstIP", conn.FlowKey.DestinationAddress.String())
			return
		}
